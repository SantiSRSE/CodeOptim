...
if flag>0 %no residuals, compute jacobian
    %IMAGE GRADIENT
    if rec.NDimen==2
        [dx(:,:,:,:,1,2),dx(:,:,:,:,1,1)]=gradient(xT(:,:,:,:,flag));dx(:,:,:,:,1,3)=0;
    else
        [dx(:,:,:,:,1,2),dx(:,:,:,:,1,1),dx(:,:,:,:,1,3)]=gradient(xT(:,:,:,:,flag));%Sixth dimension for directions of space
    end
    Sg=bsxfun(@times,rec.S,dx); %precompute the coils
    clear dx
        ...
        for l=1:NG(7);coefgr{2}=rec.tS.coefgr{2}(l,1):rec.tS.coefgr{2}(l,2);
            ...
            for k=1:NG(6);coefgr{1}=rec.tS.coefgr{1}(k,1):rec.tS.coefgr{1}(k,2);
                ...
                if rec.Field.lambda(2)>0 %second spatial derivative gradient
                    jF=bsxfun(@times,tR.dr2(coefgr{1},coefgr{2},coefgr{3},:,flag,k,l,n,:),rec.tS.BB2g(dim{1},dim{2},dim{3},:,flag,k,l,n,:));
                    Gd(1,1,1,1,1,k,l,n,:)=Gd(1,1,1,1,1,k,l,n,:)+permute(multDimSum(jF,[1:3 9]),permB);
                    jF=bsxfun(@times,tR.dr11(coefgr{1},coefgr{2},coefgr{3},:,flag,k,l,n,:),rec.tS.BB11g(dim{1},dim{2},dim{3},:,flag,k,l,n,:));
                    Gd(1,1,1,1,1,k,l,n,:)=Gd(1,1,1,1,1,k,l,n,:)+permute(multDimSum(jF,[1:3 9]),permB);
                end
            end
        end
